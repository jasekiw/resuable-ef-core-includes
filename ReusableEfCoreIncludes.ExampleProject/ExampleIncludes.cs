...
{
    public static IIncludable<Company> IncludeCompany(this IIncludable<Company> source, CompanyIncludeOptions o = default) => 
        source.IncludeCompany<Company, Company>(null, o);
    
    public static IIncludable<T> IncludeCompany<T, P>(
        this IIncludable<T> source, Include<T, P>? include,
        CompanyIncludeOptions o = default) where T : class where P : Company =>
        source.IncludeIf(o.ExcludeDepartments, s => s.IncludeDepartment(q => q.IncludeManyFrom(include, r => r.Departments)));
